...
from glob import glob
 ...
 # 调用动态链接库
dll = windll.LoadLibrary("../xunfei/bin/msc_x64.dll")
 #登录参数，apppid一定要和你的下载SDK对应
login_params = b"appid =59a8ea58, work_dir = ."
...
MSP_REC_STATUS_COMPLETE = 5
 # 你的语音文件路径
filename = "../recoder/output.wav"
# filename = "../xunfei/bin/wav/iflytek01.wav"
    ...
    def isr(self, audiofile, session_begin_params):
        ...
        while recogStatus.value != MSP_REC_STATUS_COMPLETE:
            ...
            if retstr is not None:
                ...
                print(laststr)
            # print('ret:', ret.value, 'recogStatus:', recogStatus.value, 'counter:',str(counter))
            counter += 1
            time.sleep(0.2)
            counter += 1
            if counter == 500:
